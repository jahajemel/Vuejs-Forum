...
export default {
  createPost ({commit, state}, post) {
    ...
    post.userId = state.authId //get user id
    post.publishedAt = Math.floor(Date.now() / 1000) //get kohen kur eshte postuar
    ...
    firebase.database().ref().update(updates)
      .then(() => {
        commit('setItem', {resource: 'posts', item: post, id: postId}) //ruaje postin
        commit('appendPostToThread', {parentId: post.threadId, childId: postId}) //shto postin tek threadi i caktuar
        commit('appendContributorToThread', {parentId: post.threadId, childId: post.userId}) //shto userin(contributor) tek threadi
        commit('appendPostToUser', {parentId: post.userId, childId: postId}) // shto postin tek useri i cili ka shtuar postin
  ...
  createThread ({state, commit, dispatch}, {text, title, forumId}) {
    return new Promise((resolve, reject) => {
      const threadId = firebase.database().ref('threads').push().key //get id e threadit te krijuar
      const postId = firebase.database().ref('posts').push().key
  ...
  // marja e te dhenave nga forma dhe ruajtja ne firebase database
